...
function buildMicrositeFooter(microsite, accent) {
    const companyName = microsite.footer_company_name || microsite.site_title || '';
    const website = microsite.footer_website || '';
    let socials = {};
    try { socials = typeof microsite.footer_socials === 'string' ? JSON.parse(microsite.footer_socials) : (microsite.footer_socials || {}); } catch { socials = {}; }

    const socialIcons = {
        instagram: { icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="5"/><circle cx="17.5" cy="6.5" r="1.5" fill="currentColor" stroke="none"/></svg>', label: 'Instagram' },
        twitter: { icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>', label: 'X / Twitter' },
        tiktok: { icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1v-3.5a6.37 6.37 0 00-.79-.05A6.34 6.34 0 003.15 15.2a6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.34-6.34V8.87a8.28 8.28 0 004.76 1.5V6.91a4.84 4.84 0 01-1-.22z"/></svg>', label: 'TikTok' },
        youtube: { icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>', label: 'YouTube' },
    ...
    const socialLinks = Object.entries(socials)
        ...
        .map(([platform, url]) => {
            ...
            if (!s) return '';
            return `<a href="${escapeHtml(url)}" target="_blank" rel="noopener" title="${s.label}" style="color:#888;transition:color .2s" onmouseover="this.style.color='${accent}'" onmouseout="this.style.color='#888'">${s.icon}</a>`;
        }).join('');

    const hasContent = companyName || website || socialLinks;
    if (!hasContent) {
        return `<footer style="text-align:center;padding:24px;color:#aaa;font-size:12px;border-top:1px solid rgba(0,0,0,0.05)"><p>© ${new Date().getFullYear()} ${escapeHtml(microsite.site_title || '')}</p></footer>`;
    }

    return `<footer style="border-top:1px solid rgba(0,0,0,0.08);padding:40px 24px 32px;background:#fafafa">
<div style="max-width:1080px;margin:0 auto;text-align:center">
    ${socialLinks ? `<div style="display:flex;justify-content:center;gap:16px;margin-bottom:20px">${socialLinks}</div>` : ''}
    ${companyName ? `<p style="font-size:14px;font-weight:600;color:#333;margin-bottom:4px">${escapeHtml(companyName)}</p>` : ''}
    ${website ? `<p style="margin-bottom:8px"><a href="${escapeHtml(website)}" target="_blank" rel="noopener" style="font-size:13px;color:${accent};text-decoration:none">${escapeHtml(website.replace(/^https?:\/\//, ''))}</a></p>` : ''}
    <p style="font-size:12px;color:#aaa;margin-top:12px">© ${new Date().getFullYear()} ${escapeHtml(companyName || microsite.site_title || '')}. All rights reserved.</p>
</div>
</footer>`;
}
...
${bodyHtml}
${optinPopup}
${microsite ? buildMicrositeFooter(microsite, accent) : `<footer><p>© ${new Date().getFullYear()} ${escapeHtml(title.split('|')[0].trim())}</p></footer>`}
</body>
</html>`;
}
